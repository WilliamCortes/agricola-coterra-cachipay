        ...
        <aside className="lg:w-1/4 space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-border/50">
            ...
            <div className="space-y-2">
              ...
              >
                Todos
                {!categorySlug && <Badge variant="secondary" className="ml-2 bg-white/20 text-white border-0">{allProducts?.length}</Badge>}
              </button>
        ...
        <main className="lg:w-3/4">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="h-[400px] bg-white animate-pulse rounded-2xl shadow-sm border border-border/50"></div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-border/50">
              ...
              <Button onClick={() => { setSearchTerm(""); handleCategoryChange(null); }}>
